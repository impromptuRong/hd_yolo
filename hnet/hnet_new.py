...
# from .yolov5.yolo_mask import Detect as YoloDet
# from .yolov5.yolo_mask import Model as 
...
class HNet(torch.nn.Module):
    def __init__(self, configs):
        ...
        elif backbone_cfg['type'] == 'DarkNet':
            self.backbone = DarkNet(**backbone_cfg['configs'])
            feature_channels = []
        else:
            # TODO: timm output the 2x feature map, figure out a way to get rid of it.
            self.backbone = timm.create_model(backbone_cfg['type'], pretrained=True, 
                                              features_only=True, **backbone_cfg['configs'])
            feature_channels = [_['num_chs'] for _ in self.backbone.feature_info 
                                if _['stage'] in self.backbone._stage_out_idx]
        ...
        if neck_cfg['type'] == 'fpn':
            self.fpn = BasicFeaturePyramidNetwork(
        ...
        feature_maps = OrderedDict({'0': '0', '1': '1', '2': '2', '3': '3', 'p6': 'p6', 'p7': 'p7'})
        ...
        for task_id, cfg in self.configs['headers'].items():
            ...
            if cfg['type'] == 'PanopticSeg': # segmentation header
                ...
                self.headers[task_id] = YoloDet(cfg['configs'])
            else:
...
#             features = OrderedDict([('0', features)])
        ...
        features = OrderedDict({k: v.cuda(2) for k, v in features.items()})
        losses, outputs, annotations = {}, {}, {}
        for task_id, header in self.headers.items():
            # extract roi_size and feature_maps
            roi_size = self.roi_sizes[task_id]
            ...
            task_targets = to_device(task_targets, features['0'].device)
            task_outputs, task_losses = header(features, image_size, roi_size, task_targets)
            
#             # extract roi from features
#             if targets is not None:
#                 task_targets = [_['anns'][task_id] if task_id in _['anns'] else [] for _ in targets]
#                 num_anns_per_target = [len(_['anns'][task_id]) if task_id in _['anns'] else 0 for _ in targets]
#                 task_features, task_gts = self.roi_extractor(features, image_size, roi_size, feature_maps, task_targets)
#             else:
#                 task_features, task_gts = self.roi_extractor(features, image_size, roi_size, feature_maps, targets=None)
#                 num_anns_per_target = [len(task_gts)/num_images for _ in num_images]
            
#             task_outputs, task_losses = header(task_features, roi_size, task_gts)
#             task_outputs = split_by_sizes(task_outputs, num_anns_per_target)
...
#         losses.update({"seg10x_det40x_bce_loss": -torch.mean(torch.log(p))})
        ...
        # outputs = self.postprocess(outputs, roi_sizes, original_image_sizes)
